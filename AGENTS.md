...
- Methods: `list_sessions_for_repo()`, `get_review()`, `add_comment()`, `resolve_comments()`, `resolve_all_comments()`, `save_review()`
- Shared primitive: `add_comment_to_session()` is used by both the library facade and `App::save_comment()`
- Shared primitive: `resolve_comment_in_session()` finds a comment by ID across all containers and sets `resolved` flag
